...
emb = load_embeddings("data/de_fasttext/wiki.de.vec", "fasttext")
...
reader.setup_from_file("de_fastqa")

@app.route('/api/de_fastqa', methods=['POST'])
def get_relations():
    if not request.json:
        abort(400)
