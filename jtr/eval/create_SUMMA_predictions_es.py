...
emb = load_embeddings("data/es_fasttext/wiki.es.vec", "fasttext")
...
reader.setup_from_file("es_fastqa")

@app.route('/api/es_fastqa', methods=['POST'])
def get_relations():
    if not request.json:
        abort(400)
